...
<div class="container mt-5 mb-5 main">
    <h2 class="card-title">Input Backroom / Witel</h2>
    <hr>
    <form action="{{ route('backroom.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row mb-3">
            <label for="inputEmail3" class="col-sm-2 col-form-label">Nama Backroom / Witel</label>
            <div class="col-sm-10">
            <input id="search" name="name" type="name" class="form-control @error('name') is-invalid @enderror" id="inputEmail3" placeholder="Nama Backroom / Witel" value="{{ old('name') }}">
            @error('name')
